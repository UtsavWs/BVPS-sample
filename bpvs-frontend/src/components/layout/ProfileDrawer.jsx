...
const MenuItem = memo(({ icon, label, active = false }) => (
  <button
    className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-left transition ${
      active
        ? "bg-[#FBEBE7] text-[#D64B2A]"
        : "text-gray-700 hover:bg-[#FBEBE7]"
    }`}
  >
...
const ProfileDrawer = memo(({ onClose, onLogout, user, isAdmin = false }) => {
  return (
    <>
      <div className="flex flex-col w-75 md:w-80 h-full bg-white">
        ...
        <nav className="px-3 flex flex-col gap-0.5">
          <Link to="/my-profile">
            <MenuItem
              icon={<img src="/assets/logos/ProfileIcon.svg" />}
              label="My Profile"
            />
          </Link>
          <Link to="/admin/bvpsdashboard">
            <MenuItem
              icon={<img src="/assets/logos/statistics.svg" />}
              label="Stats"
            />
          </Link>
          <Link to="/members">
            <MenuItem
              icon={<img src="/assets/logos/bvpsMembers.svg" />}
              label="BPVS Members"
            />
          </Link>
          {isAdmin && (
            <Link to="/admin">
              <MenuItem
                icon={<img src="/assets/logos/shieldIcon.svg" />}
                label="Admin Panel"
              />
            </Link>
          )}
          ...
          <Link to="/settings">
            <MenuItem
              icon={<img src="/assets/logos/settingIcon.svg" />}
              label="Settings"
            />
          </Link>
        </nav>
